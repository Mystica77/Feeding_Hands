...
router.get("/agent/collection/assign/:collectionId", middleware.ensureAgentLoggedIn, async (req, res) => {
    try {
        const collectionId = req.params.collectionId;
        const collection = await Donation.findById(collectionId);

        if (collection.status !== 'assigned') {
            collection.status = 'assigned';
            collection.deadline = new Date(Date.now() + 60 * 60 * 1000); // Set a 1-hour deadline
