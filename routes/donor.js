...
router.post("/donor/donate", middleware.ensureDonorLoggedIn, async (req,res) => {
	try
	{
		...
		donation.deadline = new Date(Date.now() + 60 * 60 * 1000);
...
router.get("/donor/donations/pending", middleware.ensureDonorLoggedIn, async (req, res) => {
    try {
        ...
        // Ensure each donation has a deadline
        pendingDonations.forEach(async (donation) => {
            if (!donation.deadline && donation.status === "pending") {
                donation.deadline = new Date(Date.now() + 60 * 60 * 1000); // Set a 1-hour deadline
                await donation.save();
            }
        });
